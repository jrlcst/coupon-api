package com.jrlcst.couponapi.coupon.infrastructure.rest;

import com.jrlcst.couponapi.coupon.application.create.CreateCouponUseCase;
import com.jrlcst.couponapi.coupon.domain.model.Coupon;
import com.jrlcst.couponapi.coupon.domain.repository.CouponRepository;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.webmvc.test.autoconfigure.AutoConfigureMockMvc;
import org.springframework.http.MediaType;
import org.springframework.test.context.ActiveProfiles;
import org.springframework.test.context.bean.override.mockito.MockitoSpyBean;
import org.springframework.test.web.servlet.MockMvc;

import java.math.BigDecimal;
import java.time.LocalDateTime;
import java.util.UUID;

import static org.hamcrest.Matchers.startsWith;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.Mockito.doThrow;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class CouponV2ControllerIntegrationTest {

    @Autowired
    private MockMvc mockMvc;

    @Autowired
    private CouponRepository repository;

    @MockitoSpyBean
    private CreateCouponUseCase createCouponUseCase;

    @Test
    @DisplayName("Scenario 1 — Create coupon successfully (happy path)")
    void scenario1() throws Exception {
        final String body = """
            {
              "code": "ABC-123",
              "description": "Iure saepe amet.",
              "discountValue": 0.8,
              "expirationDate": "2026-01-05T17:14:45.180Z",
              "published": false
            }
            """;

        mockMvc.perform(post("/v2/coupon")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.data.id").isNotEmpty())
                .andExpect(jsonPath("$.data.code").value("ABC123"))
                .andExpect(jsonPath("$.data.description").value("Iure saepe amet."))
                .andExpect(jsonPath("$.data.discountValue").value(0.8))
                .andExpect(jsonPath("$.data.expirationDate").value(startsWith("2026-01-05T17:14:45.18")))
                .andExpect(jsonPath("$.data.published").value(false))
                .andExpect(jsonPath("$.message").value("Coupon created successfully!"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 2 — Create coupon with expiration date in the past")
    void scenario2() throws Exception {
        final String body = """
            {
              "code": "ABC123",
              "description": "Expired coupon",
              "discountValue": 1.0,
              "expirationDate": "2024-01-01T10:00:00Z",
              "published": false
            }
            """;

        mockMvc.perform(post("/v2/coupon")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("The expiration date cannot be in the past"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 3 — Create coupon with discount lower than minimum")
    void scenario3() throws Exception {
        final String body = """
            {
              "code": "ABC123",
              "description": "Invalid discount",
              "discountValue": 0.2,
              "expirationDate": "2026-01-05T17:14:45Z",
              "published": true
            }
            """;

        mockMvc.perform(post("/v2/coupon")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("The discount value must be at least 0.5"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 4 — Create coupon with invalid code after sanitization")
    void scenario4() throws Exception {
        final String body = """
            {
              "code": "A-1!",
              "description": "Invalid code",
              "discountValue": 1.0,
              "expirationDate": "2026-01-05T17:14:45Z",
              "published": false
            }
            """;

        mockMvc.perform(post("/v2/coupon")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("The coupon code must contain exactly 6 alphanumeric characters"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 5 — Create coupon with missing required fields")
    void scenario5() throws Exception {
        final String body = """
            {
              "code": "",
              "discountValue": 1.0
            }
            """;

        mockMvc.perform(post("/v2/coupon")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("An error occurred while processing your request"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 6 — Get coupon by id successfully")
    void scenario6() throws Exception {
        UUID id = createCoupon("GETOK1");

        mockMvc.perform(get("/v2/coupon/" + id))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(id.toString()))
                .andExpect(jsonPath("$.data.code").value("GETOK1"))
                .andExpect(jsonPath("$.message").value("Coupon found successfully!"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 7 — Get coupon that does not exist")
    void scenario7() throws Exception {
        UUID id = UUID.randomUUID();

        mockMvc.perform(get("/v2/coupon/" + id))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("Coupon not found"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 8 — Delete coupon successfully")
    void scenario8() throws Exception {
        UUID id = createCoupon("DELOK1");

        mockMvc.perform(delete("/v2/coupon/" + id))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("Coupon deleted successfully!"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 9 — Delete coupon already deleted")
    void scenario9() throws Exception {
        UUID id = createCoupon("DELTW1");
        
        // First delete
        mockMvc.perform(delete("/v2/coupon/" + id)).andExpect(status().isOk());

        // Second delete
        mockMvc.perform(delete("/v2/coupon/" + id))
                .andExpect(status().isUnprocessableContent())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("Coupon already deleted"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 10 — Delete coupon that does not exist")
    void scenario10() throws Exception {
        UUID id = UUID.randomUUID();

        mockMvc.perform(delete("/v2/coupon/" + id))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("Coupon not found"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    @Test
    @DisplayName("Scenario 11 — Unexpected internal error")
    void scenario11() throws Exception {
        doThrow(new RuntimeException("Surprise error"))
                .when(createCouponUseCase).execute(any());

        final String body = """
            {
              "code": "ABC-123",
              "description": "Iure saepe amet.",
              "discountValue": 0.8,
              "expirationDate": "2026-01-05T17:14:45.180Z",
              "published": false
            }
            """;

        mockMvc.perform(post("/v2/coupon")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isInternalServerError())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andExpect(jsonPath("$.message").value("An error occurred while processing your request"))
                .andExpect(jsonPath("$.requestId").isNotEmpty())
                .andExpect(jsonPath("$.timestamp").isNotEmpty());
    }

    private UUID createCoupon(final String code) {
        final Coupon coupon = Coupon.createNew(
                code,
                "Iure saepe amet.",
                new BigDecimal("0.8"),
                LocalDateTime.parse("2026-01-05T17:14:45.180"),
                false,
                LocalDateTime.now()
        );
        return repository.save(coupon).getId();
    }
}
