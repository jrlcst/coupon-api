...
public class GlobalExceptionHandler {
    ...
    private static final String DEFAULT_ERROR_MESSAGE = "An error occurred while processing your request";
    ...
    public ResponseEntity<DefaultResponseDto<Void>> handleMethodArgumentNotValid(
        ...
        if (ex.getBindingResult().getErrorCount() > 1) {
            return ResponseEntity.badRequest().body(
                    DefaultResponseFactory.error(DEFAULT_VALIDATION_ERROR_MESSAGE)
            );
        }
        ...
        log.error("{} error: {}", DEFAULT_VALIDATION_ERROR_MESSAGE, errorMessage);
    ...
    public ResponseEntity<DefaultResponseDto<Void>> handleUnexpectedException(Exception ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(DefaultResponseFactory.error(DEFAULT_ERROR_MESSAGE));
    }
    ...
    public ResponseEntity<DefaultResponseDto<Void>> handleHttpMessageNotReadable(
        ...
        return ResponseEntity.badRequest().body(
                DefaultResponseFactory.error(DEFAULT_ERROR_MESSAGE)
        );
    }
